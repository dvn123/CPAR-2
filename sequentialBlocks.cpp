...
int main(int argc, char *argv[]) {
  ...
  for(int thread_id = 0; thread_id < n_threads; thread_id++) {
    ...
    vector<bool> is_prime((end - start), true);
    vector<bool> is_prime_first((end - start), true);
    ...
    for (long i = prime_i; REAL_NUMBER(i)*REAL_NUMBER(i) <= end_first*2; i++) {
      if(is_prime_first[i]) {
    ...
    while(REAL_NUMBER(prime_i)*REAL_NUMBER(prime_i) < limit_t*2) {
      if(REAL_NUMBER(prime_i)*REAL_NUMBER(prime_i) > REAL_NUMBER(end)) {
        ...
        if(REAL_NUMBER(start) % REAL_NUMBER(prime_i) == 0) {
          ...
          if((REAL_NUMBER(start) - (REAL_NUMBER(start) % REAL_NUMBER(prime_i)) + REAL_NUMBER(prime_i)) % 2 == 0) {
            j = ARRAY_INDEX(REAL_NUMBER(start) - (REAL_NUMBER(start) % REAL_NUMBER(prime_i)) + 2*REAL_NUMBER(prime_i));
          } else
